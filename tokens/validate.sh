...
# 15. --text3 used as placeholder text color (WARN)
# --text3 (#6B7280) is only 2.9:1 contrast on --bg — fails WCAG AA for placeholder
# Use --text-placeholder (#8B92A5 = 6.4:1) or --input-placeholder instead
report WARN "var(--text3) as placeholder color — fails WCAG AA (2.9:1). Use --text-placeholder or --input-placeholder" \
  "$(scan 'placeholder.*--text3|--text3.*placeholder|placeholder.*color.*--text3')"

# 16. --ctrl-h used directly in components (WARN — deprecated)
# --ctrl-h is deprecated; --input-h is the canonical input/control height token
# --ctrl-h-sm and --ctrl-h-ghost are still valid for non-input controls
report WARN "var(--ctrl-h) — deprecated; use var(--input-h) for input/control height. --ctrl-h-sm/--ctrl-h-ghost still valid." \
  "$(scan 'var\(--ctrl-h\)[^-]')"
